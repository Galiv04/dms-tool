...
const approvalSchema = z.object({
  ...
  description: z
    ...
    .optional(),
  document_id: z.string().min(1, "Seleziona un documento"),
...
export default function CreateApprovalModal({ isOpen, onClose, onSuccess }) {
  ...
  const validateMutation = useMutation({
    ...
    onError: (error) => {
      ...
      // Mostra i dettagli della validazione se disponibili
  ...
  const createMutation = useMutation({
    ...
    onError: (error) => {
      const errorMessage =
        error.response?.data?.detail || "Errore durante la creazione";
      toast({
        title: "Errore",
  ...
  }, [watchedValues, validateMutation]);
  ...
  const selectedDocument = userDocuments.find(
    (doc) => doc.id === watchedValues.document_id
  );
      ...
      <DialogContent className="w-[95vw] max-w-4xl h-[90vh] flex flex-col p-0 overflow-hidden">
        <DialogHeader className="flex-shrink-0 px-4 py-3 border-b bg-background">
          ...
          <DialogDescription className="text-sm text-muted-foreground">
            Compila i campi qui sotto per creare una nuova richiesta di
            approvazione.
          </DialogDescription>
        </DialogHeader>
                      ...
                      {validationResult.valid ? (
                        <div className="space-y-1">
                          ...
                          <div className="text-xs space-y-1">
                            ...
                            <div>
                              👥 {validationResult.recipient_count} destinatari
                            </div>
                            <div>
                              🔒{" "}
                              {validationResult.approval_type === "all"
                                ? "Tutti"
                                : "Almeno uno"}
                            </div>
                          </div>
